...
class TurtlebotMappingNode(Node):
    ...
    def robot_controller(self, scan: LaserScan):
        ...
        if front < safety_distance:  # Obstacle ahead, turn
            ...
            # Encourage movement towards open space with randomness
            if open_right > open_left * exploration_bias:
                cmd.angular.z = -0.3 + random.uniform(-self.random_turn_factor, self.random_turn_factor)  # Steer right
            elif open_left > open_right * exploration_bias:
                cmd.angular.z = 0.3 + random.uniform(-self.random_turn_factor, self.random_turn_factor)  # Steer left
            else:
                cmd.angular.z = random.uniform(-self.random_turn_factor, self.random_turn_factor)  # Add slight randomness
